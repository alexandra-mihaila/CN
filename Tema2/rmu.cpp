	...
	void display(double xmin, double ymin, double xmax, double ymax)
	{
		...
		for (double x = xmin; x <= xmax; x += RX_JF)
			for (double y = ymin; y <= ymax; y += RY_JF)
			{
				CComplex z(x, y);
				int r = isIn(z);
				if (r != 0 && r > max_r) // exista in multime
				{
					max_r = r;
				}
			}
...
{
public:
	void arboreImage2(double lungime,
		int nivel,
		double factordiviziune,
		CPunct p,
		CVector v)
	{
		...
		else
		{
			...
			v.deseneaza(p, lungime);
			p1 = v.getDest(p, lungime);
			arboreImage2(lungime * factordiviziune, nivel - 1, factordiviziune, p1, v);
	...
	void afisare(double lungime, int nivel)
	{
		...
		v.deseneaza(p, 0.125);
		p = v.getDest(p, 0.125);
		arboreImage2(lungime, nivel, 0.4, p, v);
	}
};
...
{
public:
	void segmentSierpinski(double lungime, int nivel, CPunct &p, CVector v, int inverted)
	{
		CPunct p1;
		if (nivel == 0)
		{
		...
		else
		{
			...
			{
				v.rotatie(-60);
				segmentSierpinski(lungime / 2.0, nivel - 1, p, v, 1);
				p1 = v.getDest(p, lungime / 2.0);

				v.rotatie(60);
				segmentSierpinski(lungime / 2.0, nivel - 1, p1, v, 0);
				p1 = v.getDest(p1, lungime / 2.0);

				v.rotatie(60);
				segmentSierpinski(lungime / 2.0, nivel - 1, p1, v, 1);
				p1 = v.getDest(p1, lungime / 2.0);
			}
			else
			{
				v.rotatie(60);
				segmentSierpinski(lungime / 2.0, nivel - 1, p, v, 0);
				p1 = v.getDest(p, lungime / 2.0);
	...
	void afisare(double lungime, int nivel)
	{
		...
		segmentSierpinski(lungime, nivel, p1, v1, 1);
	}
};
...
public:
	void patrat(double lungime, int nivel, CPunct &p)
	{
		if (nivel == 0)
		{
			...
			v.deseneaza(pp, lungime / 3);
			pp = v.getDest(pp, lungime / 3);
		...
		else
		{
			...
			CPunct p3(x + lungime / 3, y - lungime / 3);
			CPunct p4(x + lungime / 3, y + lungime / 3);
			CPunct p5(x, y - lungime / 3);
			CPunct p6(x, y + lungime / 3);
			CPunct p7(x - lungime / 3, y);
			CPunct p8(x + lungime / 3, y);
			CPunct p9(x, y);

			patrat(lungime / 3, nivel - 1, p1);
			patrat(lungime / 3, nivel - 1, p2);
			patrat(lungime / 3, nivel - 1, p3);
			patrat(lungime / 3, nivel - 1, p4);
			patrat(lungime / 3, nivel - 1, p5);
			patrat(lungime / 3, nivel - 1, p6);
			patrat(lungime / 3, nivel - 1, p7);
			patrat(lungime / 3, nivel - 1, p8);
			patrat(lungime, nivel - 1, p9);
		}
	}
...
// afisare imagine 2
void Display9() {
	CArboreImage2 arbore;
	arbore.afisare(0.5, nivel);
...
// afisare imagine 3
void Display0() {
	SierpinskiTriangle s;
	s.afisare(sqrt(2), nivel);
